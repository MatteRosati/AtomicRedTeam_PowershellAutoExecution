##############################################################################################
# Lateral Movement Atomic Tests - da eseguire con powershell, con privilegi di admin
##############################################################################################

# Configurazioni
$AtomicPath = "C:\Temp\Mead\atomic-red-team-master\atomic-red-team-master\atomics"
$LogDir = "C:\Temp\Mead\AtomicLogs\LateralMovement_AsAdmin"
$ReportPath = Join-Path $LogDir "LateralMovementAsAdmin_Report.csv"

if (-not (Test-Path $LogDir)) { New-Item -ItemType Directory -Path $LogDir | Out-Null }

# Lista test da eseguire
$testsToRun = @(
        @{ Technique = "T1550.003"; TestNumber = 2 }
        @{ Technique = "T1563.002"; TestNumber = 1 }
        @{ Technique = "T1021.001"; TestNumber = 2 }
        @{ Technique = "T1021.001"; TestNumber = 3 }
        @{ Technique = "T1021.002"; TestNumber = 3 }
        @{ Technique = "T1021.002"; TestNumber = 4 }
        @{ Technique = "T1072"; TestNumber = 1 }
        @{ Technique = "T1021.006"; TestNumber = 1 }
        @{ Technique = "T1021.006"; TestNumber = 3 }
)
